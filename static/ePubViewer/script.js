...
let App = function (el) {
    ...
    this.qs("#bookmarkFormCreateButton").addEventListener("click", (e) => {
        this.qs('#bookmarkForm').classList.add('bookmark-form--active')
        e.target.classList.add('inactive')
    })

    this.updateLocationHandle = (e) => {
        this.state.rendition.display(e.currentTarget.dataset.pos)
    }

    this.delete_bookmark = (e) => {
        e.preventDefault()
            e.stopPropagation()
            ...
            return fetch('/api/bookmark', {
                ...
                if(res.status === 200) {
                    const li = this.qs(`#bookmark_${id}`)
                    li.remove()
                }
            })
    }

    Array.from(this.qsa(".bookmark-delete__btn")).forEach((v) => {
        v.addEventListener('click', this.delete_bookmark)
    })


    

    this.qs("#bookmarkForm").addEventListener("submit", (e) => {
        e.preventDefault()
        const input = this.qs('#bookmarkFormInput')
        const button = this.qs('#bookmarkFormCreateButton')
        button.classList.remove('inactive')
        e.target.classList.remove('bookmark-form--active')
        return fetch('/api/bookmark', {
            method: 'POST',
            body: JSON.stringify({
                book_id: current_book.pk,
                name: input.value,
                location: this.state.rendition.currentLocation().start.cfi
            }),
            headers: {
                'Content-Type': 'application/json'
            }
        }).then((res) => res.json()).then((data) => {
            const page = this.state.book.locations.locationFromCfi(data[0].fields.location)
            const element = `
                <li id="bookmark_${data[0].pk}" class="bookmark-list-element" data-pos="${data[0].fields.location}">
                <div class="bookmark-left">
                    <span>${data[0].fields.name}</span>
                    <span class="bookmark-page">
                        (${page} стр.)
                    </span>
                </div>
                <div class="bookmark-right">
                    <button class="bookmark-delete__btn" data-id="${data[0].pk}">
            ...
            const list = this.qs('#bookmarkList')
            list.innerHTML = element + list.innerHTML
            Array.from(list.children).forEach((el) => {
                el.addEventListener("click", this.updateLocationHandle)
            })
            Array.from(this.qsa('.bookmark-delete__btn')).forEach((v) => {
                v.addEventListener('click', this.delete_bookmark)
            })
    ...
    try {
        ...
        this.qs(".bar .loc").addEventListener("click", event => {
            try {
                let answer = prompt(`Страница для перехода (до ${this.state.book.locations.length()})?`, this.state.rendition.currentLocation().start.location);
                if (!answer) return;
                answer = answer.trim();
                if (answer == "") return;

                let parsed = parseInt(answer, 10);
                if (isNaN(parsed) || parsed < 0) throw new Error("Несуществующая страница: число должно быть положительным");
...
App.prototype.doBook = function (url, opts) {
    this.qs(".book").innerHTML = "Загрузка...";
    ...
    this.state.rendition.on("touchstart", this.onRenditionDisplayedTouchSwipe.bind(this));
    this.state.rendition.on("touchend", this.onRenditionDisplayedTouchSwipe.bind(this));
...
App.prototype.doOpenBook = async function () {
    const search_params = new URLSearchParams(window.location.search)
    const response = await fetch(`/api/book/${search_params.get('id')}`)
    const fi = await response.arrayBuffer()

    var arr = (new Uint8Array(fi)).subarray(0, 2);
    var header = "";
...
App.prototype.fatal = function (msg, err, usersFault) {
    ...
    document.querySelector(".app .error").classList.remove("hidden");
    document.querySelector(".app .error .error-title").innerHTML = "Error";
    document.querySelector(".app .error .error-description").innerHTML = usersFault ? "" : "Повторите попытку.";
...
App.prototype.onBookReady = function (event) {
    ...
    if (stored) {
        this.state.book.locations.load(stored);
        [...this.qs("#bookmarkList").children].forEach(async (el) => {
            const page = this.state.book.locations.locationFromCfi(el.dataset.pos)
            const pageElement = el.querySelector('.bookmark-page')
            pageElement.innerHTML = `(${page} стр.)`
            el.addEventListener("click", this.updateLocationHandle)
        })
    }
    else this.state.book.locations.generate(chars).then(() => {
...
App.prototype.onRenditionDisplayedTouchSwipe = function (event) {
    ...
    if (touchend!=0 && touchstart!=0) {
        // if (touchend < touchstart) this.state.rendition.next();
        // if (touchend > touchstart) this.state.rendition.prev();
        if (touchend-touchstart<-100) this.state.rendition.next();
        if (touchend-touchstart>100) this.state.rendition.prev();
        touchstart = 0;
        touchend = 0;
...
App.prototype.onRenditionRelocatedUpdateIndicators = function (event) {
    try {
        ...
        if (this.getChipActive("progress") == "none") {
            ...
            stxt = `Страница ${event.start.location}/${this.state.book.locations.length()}`
...
App.prototype.onRenditionRelocatedSavePos = function (event) {
    ...
    current_book.fields.last_location = event.start.cfi
    return fetch(`/book/?id=${current_book.pk}`, {
        method: 'PUT',
        body: JSON.stringify(current_book.fields),
        headers: {
            "Content-Type": 'application/json'
        }
    }).then((res) => res.text())
...
App.prototype.onRenditionStartedRestorePos = function (event) {
    try {
        let stored = current_book.fields.last_location;
...
try {
    ...
    let ufn = window.location.search;
    if (ufn.startsWith("!")) {
        ufn = ufn.replace("!", "");
    ...
    document.querySelector(".app .error").classList.remove("hidden");
    document.querySelector(".app .error .error-title").innerHTML = "Error";
    document.querySelector(".app .error .error-description").innerHTML = "Ошибка";
    document.querySelector(".app .error .error-dump").innerHTML = JSON.stringify({
        error: err.toString(),
        stack: err.stack
    });
